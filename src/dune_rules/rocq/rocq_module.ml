...
let parse ~dir ~loc s =
  ...
  | name :: prefix ->
    ...
    let source_path = List.fold_left prefix ~init:dir ~f:Path.Build.relative in
    let source = Path.build @@ Path.Build.relative source_path (name ^ ".v") in
    let expected = Path.build @@ Path.Build.relative source_path (name ^ ".expected") in
    let expected = Some expected in
    make ~name ~expected ~source ~prefix
;;
